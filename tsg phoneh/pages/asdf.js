...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const originalData = getStoredData(); // Retrieve data from local storage
  const cartItems = [];
  ...
  const renderProducts = (originalData) => {
    if (originalData.length > 0) {
      const productsHTML = originalData
        .map(
      ...
      cartContent.addEventListener("click", (event) => {
        if (event.target.classList.contains("cart-item-remove")) {
          ...
          updateCart();
        } else if (event.target.classList.contains("cart-quantity-decrease")) {
          const itemId = event.target.dataset.itemId;
          ...
          updateCart();
        } else if (event.target.classList.contains("cart-quantity-increase")) {
          const itemId = event.target.dataset.itemId;
          const existingCartItem = cartItems.find((item) => item.id === itemId);
          if (existingCartItem) {
            existingCartItem.quantity++;
      ...
      const updateCart = () => {
        ...
        cartTotalPrice.innerHTML = `Total:${nairaFormatter.format(totalPrice)}`;
